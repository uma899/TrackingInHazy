...
# ✅ NEW SAFEE CODE:
    epoch = 1
    if args.resume != '':
              state = torch.load(args.resume) # Removed map_location since you are back on GPU!
              
              # ✅ FIX: Use .get() to prevent crashing if 'epoch' doesn't exist
              epoch = state.get('epoch', 50) 
              
              # ✅ FIX: Only load optimizer if it exists in the file
              if 'optimizer' in state:
                  optimizer.load_state_dict(state['optimizer'])
              else:
                  print("⚠️ 'optimizer' key missing in checkpoint. Using newly initialized optimizer.")

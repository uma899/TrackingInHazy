...
class UNet(nn.Module):
    def __init__(self, inchannel, outchannel, num_res) -> None:
        ...
        self.layers.append(ResBlock1(inchannel//2, outchannel//2, multi=True))
...
class Coarse2Fine(nn.Module):
    def __init__(self, k):
        ...
        for i in self.pools_sizes:
            ...
            convs.append(nn.Conv2d(k, k, 3, 1, 1, bias=False, groups=k))
    ...
    def forward(self, x):
        ...
        for i in range(len(self.pools_sizes)):
            if i == 0:
                ...
                y = self.convs[i](self.pools[i](x)+y_down)
            resl = torch.add(resl, F.interpolate(y, x_size[2:], mode='bilinear', align_corners=True))
            if i != len(self.pools_sizes)-1:
